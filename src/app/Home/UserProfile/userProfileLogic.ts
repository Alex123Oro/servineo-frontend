  ...
  <main aria-hidden="true">
      <div id="editModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="editTitle" aria-hidden="true" style="display:none">
        ...
        <div id="passwordChangeFields" style="display:none;flex-direction:column;gap:12px;margin-top:10px">
          ...
          <small id="pwHint" style="display:block;color:#666;margin-top:4px">
          Debe tener al menos 8 caracteres, incluir mayúsculas, números y símbolos.
          </small>
...
// usa siempre la versión más nueva del storage
...
if (!existingSession || isEmptySession) {
 ...
 function getUser(): User {
  ...
  // ================== LOGIN DEMO ==================
  function login() {
  // siempre forzamos a leer lo último guardado
  ...
  // si hay datos válidos, usamos esos
  const hasRealData =
    existing &&
  ...
  const u = getUser();
  if (u) {
    // ⚠️ solo cambiamos el flag, sin perder los datos actuales
    const updated = { ...u, loggedIn: false };

    // guardamos explícitamente sin borrar cambios
    ...
    if (devId) {
      ...
      // mantener todos los campos previos, solo cambiar loggedIn
  ...
  // 🔧 guardar coherente en todas partes
  ...
  // === Actualizar la visual del campo con puntos ===
