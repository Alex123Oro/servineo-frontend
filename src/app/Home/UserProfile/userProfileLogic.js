...
if (!deviceId) {
  ...
  function renderUI() {
    ...
    if (!profileIcon || !authButtons) {
      console.warn("⏳ Header no listo todavía (renderUI skipped)");
      return;
    }
  ...
  // ================== LOGIN DEMO ==================
  function login() {
    // usa mockUser como usuario de prueba
    const u = Object.assign({}, mockUser);
    u.loggedIn = true;
    setUserForDevice(u);
    // también establece en window para que Header lo pueda leer
  ...
  if (!existing || !existing.loggedIn) {
    ...
    window.userProfile = existing.loggedIn ? existing : mockUser;
    window.isAuthenticated = !!existing.loggedIn;
  } else {
